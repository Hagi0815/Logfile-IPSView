...
';
		...
		return '<!DOCTYPE html><html lang="de"><head><meta charset="utf-8">' . $refreshMeta . '<style>' . $css . $cssVar . '</style></head><body>'
			...
			.   '<button type="submit" class="btn btn-p" onclick="syncTypFilter()">&#10003; Filter</button>'
			...
			. 'function syncTypFilter(){'
			.   'var form=document.getElementById("filter-form");'
			.   'if(!form)return;'
			.   'form.querySelectorAll("input.typ-hidden").forEach(function(el){el.remove();});'
			.   'document.querySelectorAll("input.typ-cb:checked").forEach(function(cb){'
			.     'var h=document.createElement("input");'
			.     'h.type="hidden";h.name="ft[]";h.value=cb.value;h.className="typ-hidden";'
			.     'form.appendChild(h);'
			.   '});'
			. '}'
			. 'function submitTypFilter(){syncTypFilter();document.getElementById("filter-form").submit();}'
