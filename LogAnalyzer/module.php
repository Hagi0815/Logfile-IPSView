...
';
		...
		return '<!DOCTYPE html><html lang="de"><head><meta charset="utf-8">' . $refreshMeta . '<style>' . $css . $cssVar . '</style></head><body>'
			...
			.   'if(t==="heute"){zv=now.getFullYear()+"-"+p2(now.getMonth()+1)+"-"+p2(now.getDate());}'
			.   'else if(t==="1h"){var a=new Date(now-3600000);zv=fmt(a);zb=fmt(now);}'
			.   'else if(t==="6h"){var a=new Date(now-21600000);zv=fmt(a);zb=fmt(now);}'
			.   'else if(t==="7d"){var a=new Date(now-604800000);zv=fmt(a);zb=fmt(now);}'
			.   'var base="' . $h . '?a=FilterAnwenden";'
			.   'document.querySelectorAll("input.snd-cb:checked").forEach(function(cb){base+="&sf[]="+encodeURIComponent(cb.value);});'
			.   'var tf=document.querySelector("#filter-form input[name=tf]");if(tf&&tf.value)base+="&tf="+encodeURIComponent(tf.value);'
			.   'var oi=document.querySelector("#filter-form input[name=oi]");if(oi&&oi.value)base+="&oi="+encodeURIComponent(oi.value);'
			.   'document.querySelectorAll("input.typ-cb:checked").forEach(function(cb){base+="&ft[]="+encodeURIComponent(cb.value);});'
			.   'if(zv)base+="&zv="+encodeURIComponent(zv);'
			.   'if(zb)base+="&zb="+encodeURIComponent(zb);'
			.   'location.href=base;'
			. '}'
