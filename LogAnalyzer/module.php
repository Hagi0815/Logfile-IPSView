...
';
		...
		return '<!DOCTYPE html><html lang="de"><head><meta charset="utf-8">' . $refreshMeta . '<style>' . $css . $cssVar . '</style></head><body>'
			...
			.   '<select name="v" onchange="this.form.submit()" style="max-width:240px">' . $logOpts . '</select></form></div>'
			...
			.   '<a class="btn' . ($kompakt ? ' btn-p' : '') . '" href="' . $h . '?a=SetzeKompakt&v=' . ($kompakt?'0':'1') . '">&#8801; Kompakt</a></div>'
			...
			.   (($letzteSeite > 0 || $hatWeitere) ? '<a class="btn"' . ($letzteSeite > 0 && $seite >= $letzteSeite ? ' disabled' : '') . ' href="' . $h . '?a=LetzteSeite" title="Zur ältesten Seite (Ende)">&#8677;</a>' : '')
			.   '<a class="btn" href="' . $h . '?a=Aktualisieren" title="Aktualisieren (R)">&#8635;</a>'
			. '</div>'
			. '<div class="bar" style="padding:5px 10px;gap:6px">'
			.   '<span class="lbl" style="align-self:center">Export</span>'
			...
			. '<div class="bar2">'
			.   '<div class="grp" style="width:100%"><span class="lbl">Meldungstyp</span>'
			.   '<div style="display:flex;flex-wrap:wrap;gap:5px;padding-top:3px">'
			.   ($typCbs ?: '<span style="color:#555;font-size:var(--fs,12px)">Wird geladenâ¦</span>')
			.   '</div></div>'
			. '</div>'
			. '<div class="bar2">'
